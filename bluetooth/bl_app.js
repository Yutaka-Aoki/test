...
//					fs.writeFile('pulse_air.csv', 'PulseRate,PulseWave\n');
					...
					var server = new mongodb.Server('localhost,27017);	//DBê⁄ë±äÆóπ
					console.log("DBConnect");
					var db = new mongodb.Db('pulsox', server, {safe : true});
					db.open(function(err,db){
						if(err){
							throw err;
						}
						console.log("Success:DBOpen")
						var collection = db.collection('pulse_air');
...
//							fs.appendFile('pulse_air.csv', prate + ',' + pwave + '\n');
							collection.insert({prate:prated,pwave:pwaved});
